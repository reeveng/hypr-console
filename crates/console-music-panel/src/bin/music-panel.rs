//! The music, asked for.
//!
//! What it holds and why it is drawn this way is `console_music_panel::card`'s own head.
...
fn opened(asked: &[String]) -> Result<(), Never> {
    let Ok(door) = console_music_panel::door(asked);
    ...
    let Ok(drawn) = held::stood_in(console_music_panel::WHO, asked);

    match drawn {
        Drawn::ByTheHost => Ok(()),
        Drawn::Here => {
            let Ok(card) = console_music_panel::card(asked);

            panel::drawn_here(console_music_panel::WHO, card)
        },
    }
}
