...
//!
//! Nothing here is installed. This writes into `files/` and `legion apply`
...
fn run() -> Result<ExitCode, String> {
    let doing = match std::env::args().skip(1).collect::<Vec<_>>().as_slice() {
        ...
        other => return Err(format!("legion-theme takes --check and nothing else, not {other:?}")),
    };
    ...
    let changed = work
        ...
        .into_iter()
        .filter(|(written, body)| std::fs::read(&written.path).ok().as_deref() != Some(body.as_bytes()))
    ...
    match (doing, changed.as_slice()) {
        ...
        (Doing::Check, paths) => {
            paths.iter().for_each(|path| println!("  would rewrite {}", named(path)));
            return Ok(ExitCode::FAILURE);
        }
        (Doing::Write, paths) => {
            paths.iter().for_each(|path| println!("  wrote {}", named(path)));
        }
    }
    Ok(ExitCode::SUCCESS)
}
...
fn put(path: &Path, body: &str) -> Result<(), String> {
    ...
    std::fs::write(path, body).map_err(|fault| format!("{} could not be written: {fault}", path.display()))
}
        ...
        short => Some(
            short
                ...
                .map(|row| {
                    format!(
                        "  {} on {}: asked {}:1, got {:.2}:1 ({})",
                        row.front, row.back, report::ratio(row.asked), row.got, row.where_
                    )
                })
...
fn repository() -> Result<PathBuf, String> {
    ...
    here.ancestors()
        ...
        .ok_or_else(|| {
            format!("no theme/palette.toml above {}; run this inside the repository", here.display())
        })
}
