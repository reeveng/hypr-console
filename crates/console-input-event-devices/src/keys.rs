//! Every key and button the kernel has a number for, with the header's name.
//!
//! Two numbers have two names in the header -- `KEY_DIRECTION` is also
//! `KEY_ROTATE_DISPLAY`, `KEY_ZOOM` is also `KEY_FULL_SCREEN` -- and either
//! name reads back as the number; the one written first is the one printed.
...
mod tests {
    ...
    #[test]
    fn a_key_and_a_button_are_both_keys() {
        assert_eq!(KeyCode::from_str("KEY_VOLUMEUP"), Ok(KeyCode::KEY_VOLUMEUP));
        assert_eq!(format!("{:?}", KeyCode::BTN_SOUTH), "BTN_SOUTH");
        assert_eq!(format!("{:?}", KeyCode(0x2ff)), "unknown key 767");
    }

    #[test]
    fn a_number_with_two_names_reads_back_from_either() {
        assert_eq!(KeyCode::from_str("KEY_FULL_SCREEN"), Ok(KeyCode::KEY_ZOOM));
        assert_eq!(format!("{:?}", KeyCode::KEY_FULL_SCREEN), "KEY_ZOOM");
    }
}
