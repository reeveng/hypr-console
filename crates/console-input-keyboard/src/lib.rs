...
//! the copyleft with it, and the license is not a field in a manifest that can
...
//! license with a network clause in it. So the two sit beside each other, which
//! the GPL's own section 13 is written to allow, and this crate keeps the
//! license it was given.
//!
...
//!   colors as a long command line. The binding is the contract that makes
...
//! does: `dressed` in the binary puts the palette's colors in front of the
//! command line the unit gave it. What is left of the split is this one --
//! `palette` compiles without the runtime, so which color every key is can be
