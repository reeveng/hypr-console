...
//!
//! It is also where the daemon's own share of a wait is written down. Every
//! other stopwatch on this device starts when a program is exec'd, so the
//! stretch between the pad saying something and this deciding to act on it was
//! the one nobody could see -- and it is the stretch that says whether a slow
//! opening is the daemon or the toolkit.
//!
//! A turn happens twenty times a second and almost all of them decide nothing,
//! so almost all of them write nothing. A turn that starts a program always
//! writes, because that is a press somebody is waiting on the far end of. A
//! turn that only scrolled or told the home screen something writes only if it
//! took longer than a frame, which is the only version of it worth reading.
...
use console_gamepad::jobs::Rebound;
...
fn main() -> std::process::ExitCode {
    ...
    loop {
        ...
        let Ok(mut waiting) = console_wait_times::Waiting::here("controller", "press");
...
impl Bound {
    fn of(home: &str) -> Result<Self, Never> {
        let Ok(at) = console_gamepad::jobs::path_in(home);
    ...
    fn look(&mut self, turning: &mut Turning) -> Result<(), String> {
        ...
        match console_gamepad::jobs::Jobs::read(&said) {
            Ok(jobs) => {
...
fn watching() -> Result<Receiver<()>, String> {
    ...
    console_onscreen::watching_layers(say)?;
...
fn run(argv: &[String]) -> Result<Option<LetGo>, Never> {
    ...
    let Ok(()) = console_wait_times::pressed(&mut starting, "pad");
