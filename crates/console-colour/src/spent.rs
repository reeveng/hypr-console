...
//!
//! **Why the path is relative, and found from the program asking.** `SPENT` is
//! written without a leading slash on purpose. There is more than one of these
//! trees: the device has one at `/`, and the nested desktop stages a whole copy
//! of it under a directory of its own so that a machine which is not the device
//! can run the device's software without being allowed to write to `/usr`. A
//! program that joined `SPENT` onto `/` read the wrong tree in the second one --
//! the keyboard in the staged session found no palette at all, kept the colours
//! it was compiled with, and drew a keyboard nothing on this desktop is the
//! colour of. It said so on stderr, where nothing was reading.
//!
//! So the tree is the one the running program is installed in, and `beside`
//! is how it is asked. Both answers are the same file on the device, which is
//! the point: the stage differs from the device in where it is and in nothing
//! else.
