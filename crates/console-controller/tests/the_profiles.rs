//! The rules `controller-profile` and the profiles keep to each other.
//!
//! Every word the switcher takes has to name a profile that will be on the
//! machine, and one of the two is not in this repository at all: the router is
//! written by `console buttons` out of what this device says it can send. So
//! the check is not "is the file here" for both -- it is "is it here, or is it
//! the one thing the engine writes", and a third path appearing that is
//! neither is what this is for.
//!
//! These came from `console-gamepad/tests/the_button_contract.rs`, where they
//! read the switcher's shell source and looked for paths in the text. It is a
//! program now, and this asks it.

use std::path::{Path, PathBuf};

use console_controller::profile::Which;
