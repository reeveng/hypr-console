...
pub const ROLES: [&str; 16] = [
    "night", "ground", "panel", "edge", "text", "soft",
...
pub fn everywhere(files: &Path, palette: &Palette, terminal: &Terminal) -> Vec<Written> {
    let home = files.join("home/player");
    let chrome = home.join(".mozilla/firefox/legion/chrome");
    let whole = |path: PathBuf, body: String| Written { path, how: How::Whole, body };
    let region = |path: PathBuf, body: String| Written { path, how: How::Region, body };

    vec![
        // The palette, once per language.
        whole(home.join(".config/legion/palette.css"), gtk::spend(palette)),
        whole(home.join(".config/legion/palette.toml"), alacritty::spend(terminal)),
        whole(chrome.join("palette.css"), firefox::stylesheet(palette)),
        whole(files.join("usr/local/lib/legion/palette.sh"), shell::spend(palette)),
        ...
        region(home.join(".mozilla/firefox/legion/user.js"), firefox::prefs(palette)),
        ...
        region(home.join(".config/hypr/hyprland.lua"), hyprland::spend(palette)),
        // Drawn.
        whole(files.join("usr/share/icons/legion-placeholder.svg"), icon::spend(palette)),
    ]
}
...
pub mod tests {
    ...
    fn no_file_is_written_twice() {
        ...
        paths.sort();
        let once = { let mut seen = paths.clone(); seen.dedup(); seen };
    ...
    fn nothing_written_whole_is_empty() {
        for written in spent().iter().filter(|w| w.how == How::Whole) {
            assert!(!written.body.trim().is_empty(), "{:?} is empty", written.path);
        }
    }
    ...
    fn every_language_the_desktop_speaks_gets_a_palette() {
        let paths: Vec<String> = spent().iter().map(|w| w.path.display().to_string()).collect();
        for wanted in [
            ".config/legion/palette.css",   // GTK, and everything that imports it
            ".config/legion/palette.toml",  // the terminal
            "chrome/palette.css",           // the browser
            "usr/local/lib/legion/palette.sh", // the keyboard
        ] {
            assert!(paths.iter().any(|p| p.contains(wanted)), "{wanted} is not written");
        }
    }
