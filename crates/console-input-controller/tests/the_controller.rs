...
fn the_paddle_held_goes_on_scrolling() {
    let (mut go, mut daemon) = desktop();
    go.button_down("right-paddle-bottom").expect("a paddle");
...
fn the_paddle_let_go_of_stops_scrolling() {
    ...
    daemon.run(&mut go, 2);
    let so_far = of_kind(&daemon, WHEEL);
    daemon.run(&mut go, 60);
    assert_eq!(of_kind(&daemon, WHEEL), so_far, "it went on scrolling with nothing on it");
}
...
fn read_the_pad(go: &mut Go, returning: &mut Returning, now: f64) {
    ...
    for event in arrived {
        returning.saw(event.kind, event.code, event.value, now);
    }
}
...
fn legion_left_held_comes_back_from_session_game() {
    ...
    go.button_down("legion-left").expect("a button");
...
fn held_with_another_button_it_is_a_chord_of_steams() {
    ...
    let mut returning = Returning::default();
    go.button_down("legion-left").expect("a button");
    go.button_down("b").expect("a button");
    read_the_pad(&mut go, &mut returning, 1000.0);
    assert_eq!(returning.turn(1000.0 + HELD_SECONDS), Ok(None));
}
...
#[test]
fn what_was_pressed_while_the_machine_was_asleep_is_not_acted_on() {
    let (mut go, mut daemon) = desktop();
    daemon.run(&mut go, 1);
    daemon.asleep_for(2.0);
    go.press("legion-left").expect("a button");
    daemon.run(&mut go, 2);
    assert!(daemon.did.commands.is_empty(), "it ran {:?}", daemon.did.names());
}
...
fn a_button_pressed_after_the_daemon_has_settled_is_acted_on() {
    ...
    daemon.run(&mut go, 1);
    daemon.asleep_for(2.0);
    daemon.run(&mut go, 2 + (SETTLING_SECONDS / POLL) as u32);
    go.press("legion-left").expect("a button");
    daemon.run(&mut go, 2);
    assert_eq!(daemon.did.names(), ["session-game"]);
}
...
#[test]
fn a_button_pressed_after_a_minute_of_nobody_touching_it_is_acted_on() {
    let (mut go, mut daemon) = desktop();
    daemon.run(&mut go, 1);
    daemon.idle_for(60.0);
    go.press("legion-left").expect("a button");
    daemon.run(&mut go, 2);
    assert_eq!(daemon.did.names(), ["session-game"], "waiting for a press is not being asleep");
}

#[test]
fn a_stick_pushed_after_a_minute_of_nothing_scrolls_as_far_as_a_stick_pushed_at_once() {
    let (mut go, mut daemon) = desktop();
    daemon.run(&mut go, 1);
    daemon.idle_for(60.0);
    go.stick("right-stick", Point { x: 0.0, y: -1.0 }).expect("a stick");
    daemon.run(&mut go, 11);
    assert_eq!(total(&daemon, WHEEL), 2, "the minute before it was pushed is not scrolling owed");
}

#[test]
fn a_held_brightness_after_a_minute_of_nothing_steps_once_before_it_repeats() {
    let (mut go, mut daemon) = desktop();
    go.trigger("l2", 1.0).expect("a trigger");
    daemon.run(&mut go, 1);
    daemon.idle_for(60.0);
    go.hold("dpad-right").expect("the dpad");
    daemon.run(&mut go, 2);
    assert_eq!(
        daemon.did.commands,
...
fn a_half_pushed_stick_scrolls_less_than_a_quarter_as_fast() {
    ...
    full.stick("right-stick", Point { x: 0.0, y: -1.0 }).expect("a stick");
    turning_full.run(&mut full, 44);

    let (mut half, mut turning_half) = desktop();
    half.stick("right-stick", Point { x: 0.0, y: -0.6 }).expect("a stick");
...
fn inside_the_deadzone_the_page_stays_where_it_is() {
    let (mut go, mut daemon) = desktop();
    go.stick("right-stick", Point { x: 0.0, y: -0.15 }).expect("a stick");
...
fn pushing_up_scrolls_up_and_pushing_down_scrolls_down() {
    ...
    up.stick("right-stick", Point { x: 0.0, y: -1.0 }).expect("a stick");
    reading_up.run(&mut up, 11);
    let (mut down, mut reading_down) = desktop();
    down.stick("right-stick", Point { x: 0.0, y: 1.0 }).expect("a stick");
...
fn a_finger_on_the_pad_moves_the_pointer() {
    let (mut go, mut daemon) = desktop();
    go.drag(Point { x: 200, y: 200 }, Point { x: 400, y: 200 }, 4, 0.0);
    daemon.run(&mut go, 2);
...
fn the_pointer_does_not_jump_to_where_the_finger_landed() {
    let (mut go, mut daemon) = desktop();
    go.touch_down(Point { x: 900, y: 900 });
    go.touch_up();
    daemon.run(&mut go, 2);
...
fn a_drag_across_the_pad_is_not_a_click() {
    let (mut go, mut daemon) = desktop();
    go.drag(Point { x: 100, y: 100 }, Point { x: 900, y: 900 }, 8, 0.0);
    daemon.run(&mut go, 2);
