...
fn start() -> Result<Option<SyncSender<Asked>>, Never> {
    ...
    match started {
        Ok(_) => Ok(Some(say)),
        Err(fault) => {
            eprintln!("console-wait-times: no thread to write with: {fault}");
...
fn written(store: Option<Store>, at: &Path, said: &str) -> Result<Option<Store>, Never> {
    ...
    match store.file.write_all(said.as_bytes()) {
        Ok(()) => {}
        Err(fault) => {
            eprintln!("console-wait-times: {}: {fault}", at.display());
...
fn opened(at: &Path) -> Result<Option<Store>, Never> {
    match at.parent() {
        Some(above) => match std::fs::create_dir_all(above) {
            Ok(()) => {}
            Err(fault) => {
                eprintln!("console-wait-times: {}: {fault}", above.display());
    ...
    let long = match file.metadata() {
        ...
        Err(fault) => {
            eprintln!(
                "console-wait-times: {}: how long it already is would not be said: {fault}. The \
