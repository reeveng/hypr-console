...
//! one says nothing about whether that is meant. `console-child-processes` has
...
const EXCUSED: [&str; 2] = ["console-child-processes", "console-nested-desktop"];
...
fn nothing_holds_a_child_without_saying_how_long_for() {
    ...
    assert!(
        ...
        "these hold a process without saying whether it outlives them; ask \
         console_child_processes for an Alongside or a LetGo: {holding:?}"
    );
}

#[test]
fn the_crate_that_declares_them_still_does() {
    let declaring = root().join("crates/console-child-processes/src/lib.rs");
