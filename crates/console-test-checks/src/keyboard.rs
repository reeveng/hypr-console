...
fn there(stage: &mut Device) -> Done {
    ...
    happened(came, || "the keyboard did not come up".to_string())?;

    let Ok(()) = stage.press("x");
    let Ok(went) = stage.until(away, ANSWERS);
        ...
        happened(up, || {
            format!(
                "round {round} of {RESTARTS}: the controller never loaded the router, so nothing \
                 here has been asked yet. journalctl --user -u console-input-controller says why."
            )
        })?;
        ...
        happened(came, || {
            format!(
                "round {round} of {RESTARTS}: X did not raise the keyboard. Either the daemon \
                 ...
                 journalctl --user -u console-input-keyboard says which."
            )
        })?;
