...
//! and red that no check could believe. `console_colour::spent::beside` is
...
use console_colour::spent::{beside, read};
use console_input_claim::{self as claim, CONTROLLER, Claim, Heard, Spans};
use evdev::{AbsoluteAxisCode, EventType};
use console_keyboard::config::{self, Config};
use console_keyboard::drawing::Surface;
use console_keyboard::gamepad::{self, Asked, Held, Repeats};
use console_keyboard::layout::{Drops, Kind, Which, key, mods, named, of, placed, toward, under};
use console_keyboard::paint;
use console_keyboard::surface::{Gone, Missing, Poke, Screen, Showing};
use std::time::Instant;
use console_keyboard::typing::{After, Typist};
use console_wait_times::Waiting;
...
fn dressed(argv: &[String]) -> Result<Vec<String>, Never> {
    ...
    let Ok(missing) = console_keyboard::palette::missing(&palette);
    ...
    console_keyboard::palette::argv(&palette, flags)
}
...
fn me() -> Result<std::path::PathBuf, Never> {
    Ok(match std::env::current_exe() {
        ...
        Err(why) => {
            ...
            std::path::PathBuf::from(console_keyboard::palette::VIRTUAL_KEYBOARD)
        },
    })
}
...
fn run(config: &Config, mut waiting: Waiting) -> Result<(), String> {
    ...
    let Ok(root) = console_keyboard::keymap::default_symbols_root();
    let alphabets = console_keyboard::keymap::available("evdev", &root)
