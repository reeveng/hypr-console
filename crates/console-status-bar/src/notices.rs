...
//!
//! `console-notify` holds a notification until it times out or somebody takes
...
pub fn notices(waiting: Waiting) -> Result<Says, Never> {
    ...
    match (waiting.many, waiting.wrong) {
        (0, _) => Says::new(bell, Tone::Quiet),
        (_, Wrong::No) => Says::new(bell, Tone::Plain),
        (_, Wrong::Yes) => Says::new(bell, Tone::Wrong),
    }
}
  ...
  {"id":2,"app_name":"Console","summary":"One","body":null,
   "urgency":"low","actions":{}}
    ...
    fn a_bell_with_nothing_under_it_says_so_and_carries_no_number() {
        ...
        assert_eq!(says.tone, Tone::Quiet);
        assert!(!says.icon.contains(char::is_numeric), "{:?}", says.icon);
    }
    ...
    fn a_fault_among_them_colours_the_bell() {
        let ringing = bell(Waiting { many: 3, wrong: Wrong::Yes, held_back: Quiet::Coming });

        assert_eq!(ringing.tone, Tone::Wrong);
    }
    ...
    fn quiet_and_waiting_and_held_back_are_not_drawn_the_same() {
        ...
        assert_ne!(quiet.icon, bell(Waiting { held_back: Quiet::HeldBack, ..Waiting::default() }).icon);
    }
    ...
    fn a_bell_that_is_holding_them_back_is_struck_through_and_still_says_wrong() {
        ...
        assert_eq!(held.tone, Tone::Wrong);
    }
}
