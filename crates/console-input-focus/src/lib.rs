...
//! for, and then read the keys instead of the pad. `console-buttons --identify`
//! stopped the controller daemon with `SIGSTOP` and started it again
//! afterwards. Two of those are a profile load, and a profile load destroys the
//! pad and builds another -- the fault half the comments in
//! `console-input-gamepad` are about. The third is worse: stopped is not deaf,
//! so every press made meanwhile arrived in one instant against a desktop that
//! had moved on.
//!
//! What all three wanted is the kernel's own answer. `EVIOCGRAB` says this
//! process is the one the device delivers to and nothing else receives a thing,
//! and it is a property no signal and no profile can be given: the kernel holds
//! it, and the kernel lets go when the process goes, however it goes. There is
//! nothing to restore, nothing to remember, and no window in which a program
//! that died left the machine wrong.
//!
