...
same path. `legion` is the only thing that installs any of it.

    legion check     where the machine has drifted from the manifest
    legion apply     bring it back
...
touchpad it cannot translate. `emulator/` builds all four somewhere else, from
...
profile. So what a button does can be tested in under a second on a laptop.
    ...
    make fast          every test that needs no machine at all
...
| [`docs/checks.md`](docs/checks.md) | One file to a feature, replayed oldest first |
