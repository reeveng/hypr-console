...
pub fn spend(palette: &Palette) -> String {
    ...
    let entry = |name: &str, role: &str| {
        format!("    {name:<width$} = \"rgba({}ff)\",", &palette[role])
    };
    ["local blossom = {".to_string()]
        .into_iter()
        .chain([
            entry("active", "pink"),
            entry("inactive", "edge"),
...
mod tests {
    ...
    fn every_colour_is_opaque() {
        ...
        for line in spend(&blossom()).lines().filter(|l| l.contains("rgba")) {
    ...
    fn what_is_behind_everything_is_the_deepest_ground() {
        let palette = blossom();
        assert!(spend(&palette).contains(&format!("behind   = \"rgba({}ff)\"", &palette["night"])));
    }
}
