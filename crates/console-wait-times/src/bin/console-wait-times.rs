...
//!
//!     console-wait-times              the last few thousand waits
//!     console-wait-times --last 50    the last fifty
//!     console-wait-times --all        every one the store holds
//!     console-wait-times --raw        the lines themselves, for something else to read
//!
...
use console_wait_times::{line, summary, where_};

const USAGE: &str = "usage: console-wait-times [--last N] [--all] [--raw] [--file PATH]";
    ...
    while let Some(word) = words.next() {
        match word.as_str() {
            "--last" => match words.next().map(|many| many.parse::<usize>()) {
                ...
                Some(Err(fault)) => {
                    eprintln!("console-wait-times: --last: {fault}");
                    eprintln!("{USAGE}");
