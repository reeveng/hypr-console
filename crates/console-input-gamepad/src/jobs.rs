//! What a job is bound to: a button, and whatever has to be held with it.  This
//! is the shape of an answer, not the answers themselves. What the jobs are and
//! where they sit by default is `console_input_controller::means`, because that
//! is the thing that carries them out. What is here is the vocabulary the two
//! ends share: a binding is a button and a layer, it is written as `l2 +
//! right-paddle-bottom`, and it is read back the same way.  Held in
//! `console_input_gamepad` because both ends need it and neither owns it. The
