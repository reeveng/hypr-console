...
//!
//! `console_migrations` decides what a migration is and which are outstanding;
//! this is the half that is allowed to touch the machine. The split is the same
//! one the rest of the tree keeps: the crate is arithmetic over names and can be
//! asked twice, and the answer only becomes a `mv` here.
//!
//! It runs before anything else an apply does. A migration exists because the
//! manifest stopped naming something, so the machine it runs on is the one about
//! to be brought to a manifest that does not name it -- and a sweep that ran
//! afterwards would be sweeping beside a freshly installed set of the very files
//! it is deciding about. The rename went in this order for the same reason and
//! said so: what the old names left behind goes to the attic after the new ones
//! are installed *there* only because that migration was also doing the install.
//!
//! A migration that fails stops the apply. That is not the cautious choice, it
//! is the only honest one: the next thing the apply does is install over a
//! machine whose state nobody now knows, and `console apply` is the thing people
...
use console_migrations::done::{self, KEPT};
use console_migrations::sweeping;
