...
-- The panel this desktop was drawn on: mounted portrait and rotated a quarter
-- turn into landscape, 2560 pixels along the edge the desktop runs across and
-- drawn at 1024 points, which is the 2.5.
--
-- It is a seed and not the truth. Every machine that reads this file reads these
-- numbers, and they are one machine's -- a laptop told them would come up
-- rotated at two and a half times the size. So `console apply` writes the same
-- block out of what the kernel says this machine's panel actually is, and the
-- line below reads it afterwards, where a second monitor rule wins. What stays
-- here is what a machine nothing could be learned about falls back to.
hl.monitor({
    output    = "eDP-1",
    mode      = "1600x2560@144",
    position  = "auto",
...
-- there before the first apply, and nothing there in the nested desktop either.
pcall(dofile, "/home/@user@/.config/console/hypr/monitor.lua")
