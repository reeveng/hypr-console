...
impl Program for Music {
    ...
    fn heard(state: &Standing, word: &Word<Heard>) -> Turn<Standing, Its> {
        ...
        let Ok(turn) = match heard {
            ...
            Heard::Along { by, of } => {
                let Ok(press) = along(Presses { at: state.press, many: *of }, *by);
            ...
            Heard::Chose { path, kind: _the_player_is_told_the_library_either_way } => {
...
fn along(presses: Presses, by: i32) -> Result<usize, Never> {
...
fn playing(path: &Path) -> Result<Vec<Doing<Its>>, Never> {
