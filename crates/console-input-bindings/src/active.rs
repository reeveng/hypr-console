//! Which input someone is using, and the one line that remembers it.
//!
//! The last press wins. Not a setting, not a device that is plugged in, and
//! not a mode anyone puts the machine into: whichever of the two hands last
//! did something is the one the guide reads out and the one the setup screen
//! opens on. A keyboard on the desk beside a handheld no one has touched in an
...
//! events without anyone meaning anything by them -- a stick drifting on its
//! center, a trigger resting a few units off zero -- and a machine that
...
//! not. NoOne wants a live one. The guide and the setup screen are read while
//! they are up, and a page that moved under someone's eyes because they
...
//!
//! State rather than config, because no one chose it. Nothing reads it as a
...
//! answer only until the first press of a session -- someone who left the
...
pub fn of(text: &str) -> Result<Input, Never> {
    let word = text.trim();

    let found = crate::bound::EVERY.into_iter().find(|input| {
        let Ok(spelled) = input.word();
