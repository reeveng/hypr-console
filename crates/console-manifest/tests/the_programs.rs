...
//!
//! What used to hold that shut was a table here and a scan of the source under
//! it, and the scan was a net: it looked for a string literal at the front of
//! an argv and asked the machine whether a program by that name was installed.
//! It caught what it could see and missed what it could not, and it once
//! reported `info` -- an argument to `bluetoothctl` -- as a program the desktop
//! runs. The list is `console_external_programs::EVERY` now, one variant per
//! program, so nothing here has to guess: the first test below is the whole of
//! what the table was for, and it reads the enum rather than a copy of it.
//!
//! The other two are the ratchet. A program named by a string is a program the
//! enum does not know about, and a variant nothing reaches for is a package
//! nobody can justify. A crate that also has a `console_program_contract::Program`
//! in scope imports this one as `Theirs`, which is why the scan reads both
//! spellings.

use std::collections::BTreeSet;
use std::path::{Path, PathBuf};

use console_external_programs::{EVERY, Origin};
...
fn sources() -> Vec<PathBuf> {
    ...
    let declaring = root().join("crates/console-external-programs");
...
fn nothing_starts_a_program_by_writing_its_name() {
    ...
    assert!(
        strange.is_empty(),
        "these name a program instead of asking console_external_programs for one: {strange:?}"
    );
}
