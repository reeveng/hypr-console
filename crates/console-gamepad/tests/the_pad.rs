...
fn a_press_becomes_what_the_profile_says_it_is() {
    let mut pad = go(console_gamepad::router::NAME);
...
fn the_same_press_means_something_else_under_another_profile() {
    let mut routed = go(console_gamepad::router::NAME);
...
fn a_capture_written_again_is_the_file_that_is_kept() {
    let held = console_gamepad::capture::CAPTURED;
    let read: Vec<console_gamepad::capture::Descriptor> =
