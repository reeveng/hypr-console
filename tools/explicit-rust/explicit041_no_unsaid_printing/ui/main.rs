...
use console_program_contract::Doing;
...
fn says_it(what: &str) -> Doing {
    Doing::Print(what.to_string())
}
...
#[cfg_attr(
    ...
    allow(
        ...
        reason = "this is what carries `Doing::Print` out, so something here has to be the thing that prints"
    )
)]
fn carries_it_out(said: &str) -> Doing {
    println!("{said}");
