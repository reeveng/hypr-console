...
//! own a name through somebody else's program: `busctl` can call and it can
...
//!
//! It is deliberately not a D-Bus library. There is no property, no
//! introspection built out of a type, no fd passing and no client side beyond
//! what opening a connection needs -- a call goes out through `busctl` here,
//! and `console-core-external-programs` already names it. What is written is
//! what one service on one bus needs to hear a call and answer it, and the day
//! a second caller wants more, more is what it will want rather than a second
//! spelling of this.
