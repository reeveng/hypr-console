...
impl Picture {
    pub fn read(path: &Path) -> Result<Self, String> {
        ...
        let mut bands = Vec::with_capacity((width * height * 3) as usize);
        for down in 0..height as usize {
            for across in 0..width as usize {
    ...
    fn band(&self, across: u32, down: u32) -> [u8; 3] {
        let at = ((down * self.width + across) * 3) as usize;
    ...
    pub fn at(&self, across: f64, down: f64) -> Result<String, String> {
        ...
        Ok(said(self.band(across as u32, down as u32)))
    }
    ...
    pub fn average(&self, across: f64, down: f64, size: f64) -> String {
        let wide = ((f64::from(self.width) * size) as u32).max(1);
        let left = ((f64::from(self.width) * across) as u32)
            .saturating_sub(wide / 2)
            .min(self.width.saturating_sub(wide));
        let top = ((f64::from(self.height) * down) as u32)
            .saturating_sub(wide / 2)
        ...
        let seen = seen.max(1);
        said(totals.map(|total| (total / seen) as u8))
    }
    ...
    pub fn commonest(&self) -> String {
        ...
        for down in (0..self.height).step_by(every(self.height) as usize) {
            for across in (0..self.width).step_by(every(self.width) as usize) {
