...
use console_nested_desktop::staging::{Screen, Told, environment, staged};
use console_nested_desktop::talking::{Inside, Waited};
use console_nested_desktop::{screen, session, stage};
...
fn run(asked: &Asked, shot: Option<PathBuf>, probe: Doing) -> Result<u8, String> {
    ...
    match shot.is_none() && asked.seconds.is_none() && probe == Doing::Running {
        true => {
...
fn nothing_left_running(opened: Vec<(String, Child)>) -> Result<(), Never> {
    ...
    let by = std::time::Instant::now() + GOING;
...
fn stop(compositor: &mut Child, signature: &str, inside: &Inside) -> Result<(), Never> {
    ...
    while std::time::Instant::now() < by {
        match compositor.try_wait().is_ok_and(|ended| ended.is_some()) {
            true => {
                let Ok(()) = session::left_behind(signature);
