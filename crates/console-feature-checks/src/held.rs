...
use console_test_stages::device::{Device, PATIENCE, Waited};
...
fn holds_nothing(stage: &mut Device) -> Done {
    ...
    let Ok(said) = stage.user(&format!("pgrep -x {HOST} || true"));
    let running: Vec<&str> = said.lines().filter(|line| !line.trim().is_empty()).collect();

    not_empty(&running, || {
        format!("{HOST} is not up, so nothing here was asked about what it holds")
    })?;

    let Ok(said) = stage.user(&format!("pgrep -P \"$(pgrep -x {HOST} | head -1)\" -a || true"));
    let left: Vec<&str> = said.lines().filter(|line| !line.trim().is_empty()).collect();
