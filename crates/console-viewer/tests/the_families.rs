...
fn nothing_claimed_is_a_kind_this_panel_cannot_show() {
    for said in claimed() {
        assert_eq!(
            ...
            Ok(console_viewer::kinds::Shows::It),
...
fn every_decoder_a_claimed_type_needs_is_a_package_the_manifest_names() {
    ...
    let packages = console_viewer::decoding::packages().expect("what decodes what");
    let missing: Vec<&str> =
...
fn every_type_this_panel_claims_says_what_decodes_it() {
    let undecodable: Vec<String> = claimed()
        .into_iter()
        .filter(|mime| {
            console_viewer::decoding::decoder(mime).expect("what decodes it").is_none()
        })
        .collect();
