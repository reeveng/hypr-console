...
fn already(into: &Path, url: &str) -> Option<String> {
    ...
    let holding = std::fs::read_dir(into).ok()?.flatten().find_map(|entry| {
        let name = entry.file_name().to_string_lossy().to_string();
        (!getting::leftover(&name) && name.contains(&format!("[{id}]"))).then_some(name)
    })?;
...
fn swept(into: &Path, began: SystemTime) {
    ...
    for entry in reading.flatten() {
        let name = entry.file_name().to_string_lossy().to_string();
        let made = entry.metadata().and_then(|about| about.modified()).ok();
        if getting::leftover(&name) && made.is_some_and(|made| made >= began) {
