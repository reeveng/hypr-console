...
//! somewhere else with every absolute path inside it pointed back into the copy,
//! so a line here that names `/usr/local` names the staged one by the time it is
//! written -- `staging::rewritten` is the whole of that, and this file is
//! written through it rather than around it.
//!
//! The keyboard is the reason that is a rule and not a habit. This script used
//! to start it by its bare name off the staged `PATH`, which is one program
//! started the way nothing on the device starts anything: `keyboard-toggle`
//! signals a command line, that command line was the name and not the path, and
//! X did nothing on this stage for as long as it existed. The command is read
//! off `console-keyboard.service` now, so the flags the device types with are
...
pub const UNIT: &str = "files/etc/systemd/user/console-keyboard.service";
...
pub fn session_start(keyboard: &str) -> Result<String, Never> {
    Ok(SESSION_START.replace(MARK, keyboard))
}
...
# matters as much as the flags do: `console_keyboard::asked` finds the keyboard beside
...
pub fn config(screen_said: &str, device_config: &str) -> Result<String, Never> {
    Ok(format!(
        "\
...
mod tests {
    ...
    fn the_nested_config_reads_the_devices_own() {
        let said = config("-- a screen", "/somewhere/hyprland.lua").expect("the config");
