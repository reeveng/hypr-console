...
//! PipeWire that belongs to a session.
//!
//! What is here is everything that touches this machine, and it is the three
//! effects `console_dictation::comparing` decided but could not name: engines
...
use console_dictation::comparing::{Compare, Found, Heard, Its, Seen, Took};
...
fn main() -> ExitCode {
    let Ok(kept) = console_dictation::kept();
    let Ok(here) = here();
