...
//! `console-manifest/tests/the_children.rs` holds shut. A `Command` that is
//! run to completion with `status` or `output` is nobody's business here: it is
//! over before the call returns and cannot be left behind.
