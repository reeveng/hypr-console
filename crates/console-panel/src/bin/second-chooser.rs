...
use console_panel::chooser::{Again, alone, drawn, gone};
...
fn main() {
    ...
    match asked.first().map(String::as_str) {
        Some("hold") => {
            assert!(alone(&name, Again::Closes), "something was already holding it");
            drawn();
            println!("held");
        ...
        Some("twice") => println!("{} {}", said(alone(&name, Again::Closes)), said(alone(&name, Again::Closes))),
