...
- [`console-bar-modules`](crates/console-bar-modules) -- what the bar says about the machine
- [`console-browser-extension`](crates/console-browser-extension) -- the add-on this desktop puts in its browser
- [`console-button-layout`](crates/console-button-layout) -- where the buttons are on a device that is not this one
- [`console-child-processes`](crates/console-child-processes) -- how long a started program lives, said at the call site
- [`console-colour`](crates/console-colour) -- colours, and how far apart two of them are
- [`console-controller`](crates/console-controller) -- input in, a doing out; it opens no device
- [`console-cpu-boost`](crates/console-cpu-boost) -- the processors, asked to hurry while somebody waits
- [`console-defaults`](crates/console-defaults) -- what this desktop opens things with
- [`console-dictation`](crates/console-dictation) -- speaking instead of typing
- [`console-download`](crates/console-download) -- something off the net, into the folders this device plays out of
- [`console-event-broker`](crates/console-event-broker) -- one subscription per source, and everybody else is told
- [`console-external-programs`](crates/console-external-programs) -- every program this desktop runs and did not write
- [`console-feature-checks`](crates/console-feature-checks) -- the checks, one module to a feature
- [`console-file-writing`](crates/console-file-writing) -- writing a file so a machine that stops has a whole one
- [`console-files`](crates/console-files) -- the files, as something the front of the machine can walk
- [`console-flow-tests`](crates/console-flow-tests) -- the long way round, on purpose
- [`console-gamepad`](crates/console-gamepad) -- a Legion Go you can press, on a machine that is not one
- [`console-guide`](crates/console-guide) -- what every button does
- [`console-home-screen`](crates/console-home-screen) -- what is on the wallpaper, and where the thumb is on it
- [`console-input-claim`](crates/console-input-claim) -- input, claimed by whatever is in front of you
- [`console-keyboard`](crates/console-keyboard) -- the on-screen keyboard, and the one crate with a licence of its own
- [`console-launcher`](crates/console-launcher) -- the program the menu is
- [`console-manifest`](crates/console-manifest) -- `console`, the apply engine
- [`console-migrations`](crates/console-migrations) -- what a machine has to be told, because the manifest cannot say it
- [`console-music`](crates/console-music) -- the music player: what it draws, what it lists, what it presses
- [`console-nested-desktop`](crates/console-nested-desktop) -- this desktop, in a window, on another machine
- [`console-never`](crates/console-never) -- the error of a function that cannot fail
- [`console-notifications`](crates/console-notifications) -- what the desktop has said, kept where somebody can look
- [`console-number-conversion`](crates/console-number-conversion) -- the one place a number changes width
...
- [`console-publish`](crates/console-publish) -- builds this copy
- [`console-reconnect`](crates/console-reconnect) -- reaching again for something that has gone
...
- [`console-theme`](crates/console-theme) -- spends the palette into every file that holds a colour
- [`console-translation`](crates/console-translation) -- everything a person reads, in the language they read
- [`console-viewer`](crates/console-viewer) -- a photograph and a film, on the machine that holds them
- [`console-virtual-pointer`](crates/console-virtual-pointer) -- a pointer made of nothing, for the checks to press with
- [`console-virtual-touchscreen`](crates/console-virtual-touchscreen) -- a finger put down at a place on the picture
- [`console-wait-times`](crates/console-wait-times) -- how long the machine kept somebody waiting
...
The exception is [`crates/console-keyboard`](crates/console-keyboard), which is
GPL-3.0-or-later. It is a port of [wvkbd](https://git.sr.ht/~proycon/wvkbd) and
carries the licence it was given; the AGPL is not a later version of the GPL,
so it was never ours to move. Its own module comment says so.
