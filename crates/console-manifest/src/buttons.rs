...
//!
//! The deciding is all in `console_gamepad`, which can be asked without a machine.
...
use console_gamepad::front::{DEVICES, Front, asking, loading, one_said, wearing};
use console_gamepad::devices::Has;
use console_gamepad::jobs::{Jobs, Played, path_in};
use console_gamepad::router::{FILE, PROFILES, Router};
...
pub fn standing(_root: &Path, home: &str) -> Result<Standing, Never> {
    ...
    for (job, bound) in every {
        let played: Vec<&console_gamepad::jobs::Binding> = bound
            .iter()
            .filter(|one| {
