...
fn opened(asked: &[String]) -> Result<(), Never> {
    let Ok(door) = console_calendar::door(asked);
    ...
    let Ok(drawn) = held::stood_in(console_calendar::WHO, asked);

    match drawn {
        Drawn::ByTheHost => Ok(()),
        Drawn::Here => {
            let Ok(card) = console_calendar::card(asked);

            panel::drawn_here(console_calendar::WHO, card)
        },
    }
}
