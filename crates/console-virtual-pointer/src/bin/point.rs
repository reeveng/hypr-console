...
//!
//! Where ACROSS and DOWN are a place on the picture, in the size the desktop is
//! laid out in -- the numbers `hyprctl layers` answers in and the numbers
...
//! go green just as readily as the right answer does.
//!
//! The two questions are asked of the same walk `console_onscreen` already
//! makes for "is this up", so a surface this can be measured against is exactly
//! a surface the rest of the desktop agrees is on the screen.
//!
//! It asks the compositor for a pointer of its own rather than making one out
//! of uinput, and `console_virtual_pointer` says why. What that buys is the nested
//! desktop: the emulator is a compositor inside a window on this machine, a
//! uinput mouse would move the laptop's pointer and never the one in the
//! picture, and a pointer asked for over the socket is inside the picture where
//! the checks are looking. So the same command presses the device and presses
//! the emulator, and a check that could only be written for one of them can now
//! be written for both.
//!
//! The pointer stays where it is put. The virtual pointer goes when this
//! program does, and the compositor keeps the cursor where the last motion left
//! it, so a screenshot taken seconds later still has the pointer standing on
//! whatever this stood it on.

use std::time::{Duration, Instant};

use console_never::Never;
use console_number_conversion::fitted;
use console_virtual_pointer::{
