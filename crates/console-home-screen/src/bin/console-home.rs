...
impl Screen {
    ...
    fn pointed(self: &Rc<Screen>, spot: Spot, at: (f64, f64)) -> Result<(), Never> {
        let Some((was, from)) = self.seen.replace(Some((spot, at))) else { return Ok(()) };
    ...
    fn on_the_squares(&self, x: f64, y: f64) -> Result<On, Never> {
        let Some(squares) = self.grid.compute_bounds(&self.window) else { return Ok(On::Nothing) };
    ...
    fn press(self: &Rc<Screen>, reached: Reached) -> Result<(), Never> {
        ...
        let Some(name) = self.standing(here)? else {
    ...
    fn lift(self: &Rc<Screen>) -> Result<(), Never> {
        ...
        let Some(name) = self.standing(from)? else { return self.place() };
    ...
    fn put_down(self: &Rc<Screen>) -> Result<(), Never> {
        let Some(Carrying { name, from }) = self.carrying.replace(None) else { return Ok(()) };
    ...
    fn ways(self: &Rc<Screen>) -> Result<(), Never> {
        ...
        let Some(name) = self.standing(self.here.get())? else { return self.place() };
    ...
    fn room(&self) -> Result<(i32, i32), Never> {
        ...
        let Some(display) = gdk::Display::default() else { return Ok((0, 0)) };
    ...
    fn dressed(self: &Rc<Screen>) -> Result<(), Never> {
        ...
        let Some(display) = gdk::Display::default() else { return Ok(()) };
    ...
    fn reread(self: &Rc<Screen>) -> Result<(), Never> {
        ...
        let at = console_home_screen::file(&home)?;
...
fn holds_a_window() -> Result<Holds, Never> {
    ...
    let Ok(said) = asking.args(["activeworkspace", "-j"]).output()
    else {
        return Ok(Holds::AWindow);
    };
    ...
    Ok(match workspace.get("windows").and_then(serde_json::Value::as_i64).unwrap_or(1) {
        0 => Holds::Nothing,
        _ => Holds::AWindow,
    })
}
...
fn anything_over_it() -> Result<Over, Never> {
    let Ok(screens) = console_panel::door::screens() else { return Ok(Over::Something) };
...
fn worth_asking_after(line: &str) -> Result<Over, Never> {
    ...
    Ok(match ASK.iter().any(|word| line.starts_with(word)) {
        true => Over::Something,
        false => Over::Nothing,
    })
}
...
fn asked_shape() -> Result<Shape, Never> {
    ...
    let at = shape::at(&home)?;

    match std::fs::read_to_string(&at) {
        Ok(said) => Shape::read(&said),
        Err(fault) if fault.kind() == std::io::ErrorKind::NotFound => Ok(Shape::USUAL),
        Err(fault) => {
            eprintln!("console-home: {}: {fault}", at.display());
