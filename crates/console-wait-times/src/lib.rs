...
fn asked(name: &str) -> Result<Option<String>, Never> {
    match std::env::var(name) {
        ...
        Err(fault) => {
            eprintln!("console-wait-times: {name}: {fault}");
...
impl Waiting {
    ...
    pub fn done(self) -> Result<(), Never> {
        ...
        let load = match load() {
            Ok(load) => load,
            Err(fault) => {
                eprintln!("console-wait-times: {fault}");
                0.0
            }
        };
...
pub fn since_exec() -> Result<Option<Duration>, Never> {
    ...
    let ticks_f = match i32::try_from(ticks) {
        ...
        Err(fault) => {
            eprintln!("console-wait-times: the clock ticks {ticks} times a second: {fault}");
...
fn since_boot() -> Result<Option<Duration>, Never> {
    ...
    let (Ok(seconds), Ok(nanoseconds)) =
        ...
        eprintln!("console-wait-times: the boot clock said {}s {}ns", when.tv_sec, when.tv_nsec);
...
pub fn started_at(stat: &str) -> Result<Option<f64>, Never> {
    ...
    match field.parse() {
        ...
        Err(fault) => {
            eprintln!("console-wait-times: /proc/self/stat: field 22 is {field:?}: {fault}");
...
pub fn monotonic_now() -> Result<Option<Duration>, Never> {
    ...
    let (Ok(seconds), Ok(nanoseconds)) =
        ...
        eprintln!("console-wait-times: the monotonic clock said {}s {}ns", when.tv_sec, when.tv_nsec);
