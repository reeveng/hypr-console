...
//! in the manifest like any other package, and `legion apply` builds before it
...
mod tests {
    ...
    fn a_program_is_installed_where_everything_not_from_a_package_lives() {
        assert_eq!(live("legion-panel"), "/usr/local/bin/legion-panel");
    }
    ...
    fn cargo_is_asked_once_for_every_program() {
        ...
        let names = ["legion-panel".to_string(), "legion-buttons".to_string()];
        assert_eq!(
            how(&names),
            [
                "build", "--release", "--locked",
    ...
    fn a_program_nothing_has_compiled_is_unbuilt_whatever_the_machine_holds() {
        ...
        assert_eq!(state(nowhere, "legion-panel"), State::Unbuilt);
    }
}
