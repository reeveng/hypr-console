...
impl Wash {
    ...
    pub fn of(code: &str, alpha: f64) -> Self {
        let channel =
            |at: usize| u8::from_str_radix(&code[at..at + 2], 16).unwrap_or(0) as f64 / 255.0;
        Wash {
            red: channel(0),
            green: channel(2),
            blue: channel(4),
...
impl Paints {
    ...
    pub fn get(&self, name: &str) -> Wash {
        *self
            .0
            .get(name)
            .unwrap_or_else(|| panic!("the garden dips in {name}, which the palette does not name"))
    }

    /// One paint laid over another, by the top one's own alpha unless told.
    pub fn washed(&self, over: &str, under: &str, share: Option<f64>) -> Wash {
        let top = self.get(over);
        top.over(self.get(under), share.unwrap_or(top.alpha))
    }
}

/// Dip the brush.
pub fn dip(ctx: &Context, paint: &Paints, name: &str, alpha: f64) {
...
pub fn stop(gradient: &Gradient, offset: f64, paint: &Paints, name: &str, alpha: f64) {
    let wash = paint.get(name);
...
/// two curves.
pub fn petal_at(ctx: &Context, x: f64, y: f64, size: f64, turn: f64) {
...
mod tests {
    ...
    fn a_wash_worked_out_is_opaque() {
        ...
        let over = Wash::of("ffffff", 0.25).over(Wash::of("000000", 1.0), 0.25);
    ...
    fn a_wash_takes_its_own_alpha_unless_told_otherwise() {
        let paints = Paints::of([
            ("mist".to_string(), Wash::of("ffffff", 0.5)),
            ("earth".to_string(), Wash::of("000000", 1.0)),
        ]);
        assert_eq!(paints.washed("mist", "earth", None).red, 0.5);
        assert_eq!(paints.washed("mist", "earth", Some(0.1)).red, 0.1);
    }

    #[test]
    fn dipping_in_a_colour_the_palette_does_not_name_says_which_one() {
        let paints = Paints::default();
        let fault = std::panic::catch_unwind(|| paints.get("nothing")).expect_err("no such paint");
        let said = fault.downcast_ref::<String>().expect("a message");
