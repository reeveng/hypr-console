...
//! its offset, never a sound out of a device somebody has just switched on. So
...
//! somebody the thing they were in the middle of.
//!
...
//! offset is as good as the last press, which is what somebody pausing and
...
pub fn of(said: &str) -> Result<Option<Kept>, Never> {
...
pub fn read() -> Result<Option<Kept>, Never> {
    let Ok(where_) = where_();
...
mod tests {
    ...
    fn what_is_written_is_what_is_read_back() {
        let song = Path::new("/home/somebody/Music/a song.flac");
        let Ok(said) = said(song, 61.5);

        assert_eq!(
            of(&said),
            Ok(Some(Kept { song: song.to_path_buf(), at: 61.5 }))
        );
    }
    ...
    #[test]
    fn an_offset_nobody_can_read_is_the_start_of_the_song_and_not_a_lost_song() {
        let kept = of("[playing]\nsong=/a.flac\nat=the middle\n");

        assert_eq!(kept, Ok(Some(Kept { song: PathBuf::from("/a.flac"), at: 0.0 })));
    }
