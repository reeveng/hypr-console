...
fn opened(asked: &[String]) -> Result<(), Never> {
    let Ok(door) = console_notifications::door(asked);
    ...
    let Ok(drawn) = held::stood_in(console_notifications::WHO, asked);

    match drawn {
        Drawn::ByTheHost => Ok(()),
        Drawn::Here => {
            let Ok(card) = console_notifications::card(asked);

            panel::drawn_here(console_notifications::WHO, card)
        },
    }
}
