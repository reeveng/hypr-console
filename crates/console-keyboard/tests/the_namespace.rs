...
fn the_bin() -> String {
    let held = std::fs::read_to_string(root().join("crates/console-keyboard/Cargo.toml"))
...
fn the_toggle_and_the_keyboard_are_installed_where_one_can_find_the_other() {
    ...
    for name in ["keyboard-toggle", "keyboard-show", console_controller::mode::KEYBOARD] {
        assert!(
