//! What the controller asked the keyboard to do.
//!
//! The on-screen keyboard reads the controller itself while it is up, and that
//! is a claim rather than an arrangement: `console_input_claim` takes the pad
//! and the keyboard InputPlumber publishes beside it for exactly as long as the
//! layer surface is on the screen, with `EVIOCGRAB` underneath, so nothing else
//! receives a press meanwhile. Two readers of one device would both act on the
//! right stick, which navigates and scrolls at once and flickers.
//!
//! It used to be a profile instead -- `keyboard.yaml`, which translated
//! nothing, loaded when the keyboard came up and taken off when it went. That
//! worked and cost more than it bought: a profile load destroys the pad and
//! builds another, which is the flake where X stopped showing the keyboard
//! until the next reboot, and the `After=` on the unit that was put there to
//! survive it. The claim is the same promise made where it can be kept.
//!
...
//!
//! Which button is which is nobody's to say here. `console_input_claim` names
//! a press in the profile's own words and `console_gamepad::vocabulary` turns
//! that into the word a person uses, which is what this table is written in.
//! The trap that vocabulary exists for is live in this file: the button
//! labelled X on this device is `BTN_NORTH` -- `North` -- and the one labelled
//! Y is `BTN_WEST`, and X is the button that puts the keyboard on the screen.

use std::time::{Duration, Instant};

use console_gamepad::vocabulary::spoken_for;
use console_input_claim::{Said, Spans, Went};
...
mod tests {
    use super::*;
    use console_gamepad::vocabulary::BUTTONS as EVERY;
