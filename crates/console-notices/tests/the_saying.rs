...
/// How many times one kind of fault reaches the screen.
///
/// `console_notices::saying::LOUD` is the program's own, and this is written
/// out again rather than imported: a test that reads the number it is checking
/// out of the thing it is checking agrees with it whatever either becomes.
const LOUD: usize = 5;

/// Somewhere to be said to: a stub for each program the program reaches for,
...
impl Listening {
    ...
    fn run(&self, program: &str, argv: &[&str], result: Option<&str>) {
        // The stubs first, so notify-send and logger are the test's own.
        let path = format!(
            "{}:{}",
