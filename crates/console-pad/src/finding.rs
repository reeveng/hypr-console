...
/// Who made a device: the daemon that publishes pads, or a hand holding one.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum Made {
    /// InputPlumber published it, so it is a pad this desktop composed.
    ByInputPlumber,
    /// It is plugged in somewhere, so somebody is holding it.
    ByHand,
}
...
impl Says {
    fn has_key(&self, key: KeyCode) -> Has {
        match self.keys.contains(&key.0) {
            true => Has::Yes,
            false => Has::No,
        }
    }

    fn has_axis(&self, axis: AbsoluteAxisCode) -> Has {
        match self.axes.contains(&axis.0) {
            true => Has::Yes,
            false => Has::No,
        }
    }
...
pub fn gamepad(among: &[Says]) -> Option<&Says> {
    among.iter().find(|says| {
        says.has_axis(AbsoluteAxisCode::ABS_RX) == Has::Yes
            && says.has_axis(AbsoluteAxisCode::ABS_RY) == Has::Yes
            && says.made() == Made::ByInputPlumber
    })
}
...
pub fn keyboard(among: &[Says]) -> Option<&Says> {
    among.iter().find(|says| {
        says.has_key(KeyCode::KEY_F13) == Has::Yes
            && says.has_key(KeyCode::KEY_ESC) == Has::Yes
            && says.made() == Made::ByInputPlumber
    })
}
...
pub fn touchpad(among: &[Says]) -> Option<&Says> {
    among.iter().find(|says| {
        says.has_key(KeyCode::BTN_TOUCH) == Has::Yes
            && says.has_axis(AbsoluteAxisCode::ABS_X) == Has::Yes
