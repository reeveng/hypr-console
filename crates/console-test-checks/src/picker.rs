//! What every picker does: it draws, it leaves the pad alone, and B closes it.
//!
//! The menu, the guide and the settings panel are three programs and one
//! contract. Written once here so a fourth picker is a line of data rather
//! than a fourth copy of the same four assertions.
//!
//! The middle assertion used to be the other way round. A picker took the pad
...
use console_test_stages::checking::{CheckResult, happened, happened_handed, same};
use console_test_stages::device::{Device, PATIENCE};

pub const WORN: &str = "Router";

#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct Expected<'a>(pub &'a str);
...
pub fn opens(stage: &mut Device, button: &str, what: Expected<'_>) -> CheckResult {
    ...
    same(&held, WORN, || {
        format!("the {what} changed the profile to {held}, and a picker has nothing to change")
    })?;
