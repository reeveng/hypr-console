//! What another program asks of this keyboard from outside it.
//!
//! Two programs, one line each, and the whole of the difference between them
//! is which signal. They are here rather than in the keyboard's own binary
//! because the keyboard may not be running when somebody asks -- and if it is
//! not, saying so is the answer.
//!
//! **Why the keyboard is asked at all, rather than told.** The keyboard owns
//! the answer to "is it up", because it is the thing that is up. An earlier
//! version of `keyboard-toggle` kept that in a file and guessed wrong every
//! other press, which is the ownerless variable `docs/programs.md` is about,
//! arriving by a small road.
//!
//! **Why a toggle and a show, and not one with a flag.** A toggle is the wrong
//! shape for a program asking on somebody's behalf: the browser's search card
//! opening while the keyboard happened to be up would have put it away, which
//! is a card asking for a keyboard and taking one instead. So the half that
//! only ever shows is its own program, and `docs/browser.md` is where that was
//! settled.
//!
//! **Why the path and not the name.** `pkill -x` compares against the kernel's
//! `comm`, which is fifteen characters, and `virtual-keyboard` is sixteen:
//! `pkill -x virtual-keyboard` matches nothing at all, silently, and what that
//! looks like is X doing nothing for ever. `-f` matches the whole command
//! line, which begins with the path the unit started it as, and the anchor
//! keeps it from finding anything that merely mentions the keyboard.
//! `crates/console-manifest/tests/the_tree.rs` holds the rule that hid it.
//!
//! **Why the path is found and not written down.** It was
...
//! reason.
//!
//! So the keyboard is the one beside whoever is asking. The toggle, the show
//! and the keyboard are installed in one directory by `[build]` -- the manifest
//! puts them there and `tests/the_namespace.rs` keeps it that way -- so a
//! program that knows where it is knows where the keyboard is, on the device
//! and in a stage, without either of them being told which it is in.
