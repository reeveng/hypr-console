...
pub const STEP: std::num::NonZeroU16 = match std::num::NonZeroU16::new(1) {
    Some(step) => step,
    None => std::num::NonZeroU16::MIN,
};
...
pub fn step(percent: u16) -> Result<String, Never> {
    let at = (percent.min(WHOLE) / STEP).saturating_mul(STEP.get());

    Ok(format!("at-{at}"))
}

pub fn steps() -> Result<Vec<String>, Never> {
    Ok((0..=WHOLE / STEP).map(|step| format!("at-{}", step.saturating_mul(STEP.get()))).collect())
}
...
mod tests {
    ...
    fn the_step_never_runs_ahead_of_the_number() {
        for percent in 0..=100_u16 {
            ...
            assert!(percent - at < STEP.get(), "{percent}% is painted as {at}%, a whole step short");
        }
    }
