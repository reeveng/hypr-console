//! The colors as the machine spends them.
//!
//! Read out of the file every themed surface is themed from, so a palette that
//! moves moves its checks with it. A check carrying its own copy of a color is
//! a check that goes red for someone else's good reason, or worse, stays green
//! against a color nothing uses any more.

use std::collections::BTreeMap;

pub use console_core_color::palette::read;
...
mod tests {
    ...
    fn the_palette_this_machine_spends_is_read_off_the_file_that_spends_it() {
        ...
        assert!(!found.is_empty(), "no colors in {SPENT}");
    }
}
