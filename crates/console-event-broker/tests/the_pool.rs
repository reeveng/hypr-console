...
use console_event_broker::listening::listen_at;
use console_event_broker::serving;
use console_event_broker::sources::Held;
...
fn source(topic: &Topic, say: Sender<Changed>) -> Result<Held, console_never::Never> {
    Ok(match topic {
...
fn before_long(heard: &Receiver<Changed>) -> Option<Changed> {
    heard.recv_timeout(BEFORE_LONG).ok()
}
