//! The layer above the devices: one event, in one vocabulary.
//!
//! What a program wants to know is which button a person pressed. Which device
//! file said so is an accident of how the controller was published this boot,
//! and every program that has had to know it has been wrong about it at least
//! once -- the paddles arrive as function keys, X arrives as a function key,
//! and everything else arrives as itself, so a reader that knew only the pad
//! silently missed a third of the front of the machine.
//!
//! So a press is `South` here, and a program that wants to say what A does
//! never learns that A is `BTN_SOUTH` on one node and might not be on the next.
//! `console_gamepad::routing` is the table that says which is which and
//! `console_gamepad::vocabulary` turns a profile's word into a person's; both
//! are shared with the daemon rather than written twice.
//!
//! Nothing here opens anything or holds any state. The same event named twice
//! is named the same way, which is what makes a transcript of somebody pressing
//! buttons a test.

use console_gamepad::routing::{self, Hat};
use console_gamepad::vocabulary::TRIGGER_BUTTONS;
