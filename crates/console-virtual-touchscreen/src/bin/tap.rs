...
//! `console_screen::Screen::on_the_panel` is the arithmetic, and it is the same
//! arithmetic either way round.
//!
//! The device used to be made here by hand: /dev/uinput opened as a file, the
//! ioctl request numbers derived at compile time, `input_event` packed into
//! twenty-four bytes and written. All of that was already in the tree.
//! `console_gamepad::uinput` builds the emulator's pad through `evdev`, the
//! daemon publishes its wheel through `evdev`, and `evdev` is on the device
//! because both of them are. Two answers to how this machine makes an input
//! device is one more than it can keep true: a kernel that changed what a
//! multi-touch device has to declare would be found by one of them and not the
//! other. This is the one that was written twice, so this is the one that
//! stopped.
//!
