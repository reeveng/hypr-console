...
//!
//! Two files on this machine cannot import a palette: KDE's ini format has no
//! include and Firefox's `user.js` is a list of literals. A third, the
//! compositor's Lua, could but must not, because a Lua file that fails to load
//! takes the session with it. Those three have a pair of markers in them and
//! only what lies between the markers is ours.

pub const BEGIN: &str = "legion-theme:begin";
pub const END: &str = "legion-theme:end";
...
mod tests {
    ...
    fn a_body_is_written_with_exactly_one_newline_after_it() {
        for body in ["new", "new\n", "new\n\n\n"] {
            ...
            assert!(got.contains("new\n# legion-theme:end"), "{body:?} gave {got:?}");
        }
    }

    #[test]
    fn an_empty_region_is_filled() {
        let held = "# legion-theme:begin\n# legion-theme:end\n";
        assert_eq!(
    ...
    fn markers_in_the_wrong_order_are_refused() {
        let backwards = "# legion-theme:end\nbody\n# legion-theme:begin\n";
        assert_eq!(spliced(backwards, "new"), None);
    }

    #[test]
    fn a_half_marked_file_is_refused() {
        assert_eq!(spliced("# legion-theme:begin\nbody\n", "new"), None);
        assert_eq!(spliced("# legion-theme:end\nbody\n", "new"), None);
    }
}
