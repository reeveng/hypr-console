...
use console_input_gamepad::profile::{Kind, Source};
use console_input_gamepad::router::every_profile as load_all;
...
fn the_left_stick_moves_the_pointer_and_reaches_the_pad_as_well() {
    let profiles = load_all(&root()).expect("the profiles read");
    let router = &profiles[console_input_gamepad::router::NAME];
    let stick = router
        .mappings
        .iter()
