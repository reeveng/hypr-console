...
use console_keyboard::palette::{self, BACKGROUNDS, COLOURS, INK};
...
fn pressed_and_selected_keys_are_seen() {
    ...
    for option in ["press", "sel"] {
        ...
        let Ok(apart) = console_colour::contrast(background, dark_ink);
...
fn a_pressed_key_is_not_the_key_under_the_stick() {
    ...
    let Ok((_, _, one)) = console_colour::to_oklch(&press);
    let Ok((_, _, other)) = console_colour::to_oklch(&sel);
